<template>
  <BaseCard>
    ...
    <div class="col-8">
      <ProBraCatNavbar />
      <BaseCard>
        <div class="add_text text-start">新增种类</div>
        ...
        <div class="input_region">
          ...
          <div class="add_cat_level">
            <div class="row">
              <div class="col-3">
                <div class="add_cat_level_text text-start">新增分类级别</div>
              </div>
              <div class="col-6">
          ...
          <!-- 新增父节点名字 -->
          <div class="add_Father_cat">
            <div class="row">
              <div class="col-3">
                <div class="add_Father_cat_text text-start">新增父节点名字</div>
              </div>
              <div class="col-6">
                <div class="input-group mb-3 input-group-lg">
                  <input
                    v-model="Father_cat_name"
                    type="text"
                    class="form-control"
                    placeholder="输入新增加的结点的父节点名字"
                    aria-label="Recipient's username"
                    aria-describedby="button-addon2"
                  />
                </div>
          ...
          <div class="add_Son_cat">
            <div class="row">
              <div class="col-3">
                <div class="add_Son_cat_text text-start">新增节点名字</div>
              </div>
              <div class="col-6">
                <div class="input-group mb-3 input-group-lg">
                  <input
...
export default {
  ...
  setup() {
    ...
    const add_cat_submit = () => {
      error_message.value = '';
      if (cat_level.value != 1 && cat_level.value != 2 && cat_level.value != 3 && cat_level.value == '') {
        error_message.value += '级别输入不合法 '
      }
      if (Father_cat_name.value == '') {
        error_message.value += '父节点名称未填写 '
      }
      if (Son_cat_name.value == '') {
        error_message.value += '添加的结点名称未填写 '
      }
      if (error_message.value == '') {
        $.ajax({
          url: "http://192.168.80.3:80/api/product/category/AddNewCat",
          type: "POST",
          data: {
            catname: Son_cat_name.value,
            Fcatname: Father_cat_name.value,
